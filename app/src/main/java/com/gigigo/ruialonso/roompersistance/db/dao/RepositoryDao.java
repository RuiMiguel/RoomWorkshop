...
@Dao @TypeConverters(DateConverter.class) public interface RepositoryDao {
  @Query("SELECT * FROM Repository") LiveData<List<RepositoryEntity>> findAllRepositories();

  @Query("SELECT Repository.* FROM Repository INNER JOIN User ON Repository.user_id = User.id WHERE User.name = :username")
  LiveData<List<RepositoryEntity>> findAllRepositoriesByUsername(String username);

  @Query("SELECT Repository.* FROM Repository INNER JOIN User ON Repository.user_id = User.id WHERE User.name IN (:usernames)")
  LiveData<List<RepositoryEntity>> findAllRepositoriesByUsernamesCollection(List<String> usernames);

  @Insert(onConflict = OnConflictStrategy.REPLACE) void insertRepository(
